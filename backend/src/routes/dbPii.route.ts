  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const batch = await prisma.batch.create({
        data: {
          scanType: "DB_SCAN", // or create a new scanType if needed
          creatorId: user.id,
          totalNumFiles: 1,
        },
      });
      ...
      const dbRecord = await prisma.db.create({
        data: {
          ...
          dbType: db_type.toUpperCase(), // must match enum: POSTGRES, MYSQL, ORACLE
          batchId: batch.id,
          ...
          for (const column of table.columns) {
            await prisma.dbFullPiiResult.create({
              data: {
                ...
                classification: column.classifications as any, // must match Classification enum
