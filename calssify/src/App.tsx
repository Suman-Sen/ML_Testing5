...
const App: React.FC = () => {
  ...
  const runDbScan = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/pii", {
        method: "POST",
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-md p-8">
          <h1 className="text-3xl font-bold text-center mb-6 text-blue-700">ID Scanner</h1>
          ...
          {scanContext === "document" && (
            <>
              <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
                <label htmlFor="file-upload" className="text-sm font-medium text-gray-700">Upload Images</label>
                <input
                  id="file-upload"
                  type="file"
                  multiple
          ...
          {scanContext === "db" && (
            <div className="space-y-4">
              <label className="block text-sm font-medium text-gray-700">DB Connection String</label>
              <input
                type="text"
                value={dbConnString}
